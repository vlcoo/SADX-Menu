...
func _ready() -> void:
	...
	if bg != null and bg_fades & 1:
		...
		bg_tween.tween_property(bg, ^"self_modulate", Color.WHITE, 0.1).set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN)
...
func fade_and_change_scene_to_file(path: String) -> Error:
	if bg != null and bg_fades & 2:
		var bg_tween = create_tween()
		bg_tween.tween_property(bg, ^"self_modulate", Color.GRAY, 0.1).set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN)
